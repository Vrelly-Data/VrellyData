-- Fix seniority filtering with proper value mapping and OR/AND logic
...
BEGIN
  ...
  WHERE fd.entity_type = p_entity_type
    ...
    -- FIXED: Seniority filter with proper value mapping and strict logic
    AND (
      p_seniority IS NULL OR array_length(p_seniority, 1) IS NULL
      OR (
        -- C-Level/CxO: Match seniority field 'Cxo' OR C-suite title patterns
        (
          EXISTS (SELECT 1 FROM unnest(p_seniority) s WHERE LOWER(s) IN ('cxo', 'c-level', 'c level'))
          AND (
            LOWER(fd.entity_data->>'seniority') = 'cxo'
            OR (fd.entity_data->>'title') ILIKE '%CEO%'
            OR (fd.entity_data->>'title') ILIKE '%CTO%'
            OR (fd.entity_data->>'title') ILIKE '%CFO%'
            OR (fd.entity_data->>'title') ILIKE '%CMO%'
            OR (fd.entity_data->>'title') ILIKE '%CISO%'
            OR (fd.entity_data->>'title') ILIKE '%CIO%'
            OR (fd.entity_data->>'title') ILIKE '%CSO%'
            OR (fd.entity_data->>'title') ILIKE '%COO%'
            OR (fd.entity_data->>'title') ILIKE '%CHRO%'
            OR (fd.entity_data->>'title') ILIKE '%CRO%'
            OR (fd.entity_data->>'title') ILIKE '%Chief %'
          )
        )
        OR
        -- President: Match seniority field 'President' OR title 'President' (excluding VP)
        (
          EXISTS (SELECT 1 FROM unnest(p_seniority) s WHERE LOWER(s) = 'president')
          AND (
            LOWER(fd.entity_data->>'seniority') = 'president'
            OR (
              (fd.entity_data->>'title') ILIKE '%President%' 
              AND (fd.entity_data->>'title') NOT ILIKE '%Vice President%'
            )
          )
        )
        OR
        -- VP/Vice President: Match seniority field 'Vp' OR VP title patterns
        (
          EXISTS (SELECT 1 FROM unnest(p_seniority) s WHERE LOWER(s) IN ('vp', 'vice president'))
          AND (
            LOWER(fd.entity_data->>'seniority') = 'vp'
            OR (fd.entity_data->>'title') ILIKE '%Vice President%'
        ...
        OR
        -- Director: Match seniority field 'Director' OR title 'Director' (excluding Managing Director)
        (
        ...
        OR
        -- Head of: Match title patterns only (Head of, Principal, Managing Director)
        (
        ...
        OR
        -- Manager: Match seniority field 'Manager' OR title 'Manager'
        (
          EXISTS (SELECT 1 FROM unnest(p_seniority) s WHERE LOWER(s) = 'manager')
          AND (
            LOWER(fd.entity_data->>'seniority') = 'manager'
            OR (fd.entity_data->>'title') ILIKE '%Manager%'
          )
        )
        OR
        -- Individual Contributor: Match seniority field 'Staff' OR exclude all senior titles
        (
          EXISTS (SELECT 1 FROM unnest(p_seniority) s WHERE LOWER(s) = 'individual contributor')
          AND (
            LOWER(fd.entity_data->>'seniority') = 'staff'
            OR (
              -- If seniority is null or empty, use title-based exclusion
              (fd.entity_data->>'seniority' IS NULL OR fd.entity_data->>'seniority' = '')
              AND (fd.entity_data->>'title') NOT ILIKE '%CEO%'
              ...
              AND (fd.entity_data->>'title') NOT ILIKE '%President%'
              AND (fd.entity_data->>'title') NOT ILIKE '%Vice President%'
