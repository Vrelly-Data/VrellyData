-- ============================================================
-- v2.10: REVENUE FILTER FIX
-- Changes ONLY the company revenue filter to parse mixed-format
-- data (numeric strings + text ranges) into numeric comparisons.
-- All other logic is IDENTICAL to v2.9.
-- ============================================================

-- Step 1: Create helper function to parse revenue strings to numeric
CREATE OR REPLACE FUNCTION public.parse_revenue_to_numeric(revenue_str text)
RETURNS numeric
LANGUAGE plpgsql
IMMUTABLE
...
DECLARE
  ...
  upper_bound numeric;
BEGIN
  IF revenue_str IS NULL OR revenue_str = '' THEN
    RETURN NULL;
  END IF;
  
  clean_str := TRIM(revenue_str);
  
  -- Case 1: Pure numeric string (e.g., "10000000", "1700000000")
  IF clean_str ~ '^[0-9]+$' THEN
    RETURN clean_str::numeric;
  END IF;
  
  -- Case 2: "Under X Million" pattern
  IF clean_str ~* '^under\s+(\d+)\s+million' THEN
    upper_bound := (REGEXP_MATCH(clean_str, '(\d+)\s+million', 'i'))[1]::numeric;
    RETURN upper_bound * 500000; -- midpoint of 0 to X million
  END IF;
  
  -- Case 3: "X Million to Y Million" pattern
  IF clean_str ~* '(\d+)\s+million\s+to\s+(\d+)\s+million' THEN
    upper_bound := (REGEXP_MATCHES(clean_str, '(\d+)\s+million\s+to\s+(\d+)\s+million', 'i'))[2]::numeric;
    RETURN upper_bound * 1000000;
  END IF;
  
  -- Case 4: "X Billion to Y Billion" pattern
  IF clean_str ~* '(\d+)\s+billion\s+to\s+(\d+)\s+billion' THEN
    upper_bound := (REGEXP_MATCHES(clean_str, '(\d+)\s+billion\s+to\s+(\d+)\s+billion', 'i'))[2]::numeric;
    RETURN upper_bound * 1000000000;
  END IF;
  
  -- Case 5: "Over X Billion" pattern
  IF clean_str ~* 'over\s+(\d+)\s+billion' THEN
    upper_bound := (REGEXP_MATCH(clean_str, '(\d+)\s+billion', 'i'))[1]::numeric;
    RETURN upper_bound * 1500000000; -- 1.5x the threshold
  END IF;
  
  -- Case 6: Contains "million" with a number
  IF clean_str ~* '(\d+)\s*million' THEN
    num_val := (REGEXP_MATCH(clean_str, '(\d+)', 'i'))[1]::numeric;
    RETURN num_val * 1000000;
  END IF;
  
  -- Case 7: Contains "billion" with a number
  IF clean_str ~* '(\d+)\s*billion' THEN
    num_val := (REGEXP_MATCH(clean_str, '(\d+)', 'i'))[1]::numeric;
  ...
  IF clean_str ~ '\d' THEN
    num_val := NULLIF(REGEXP_REPLACE(clean_str, '[^0-9]', '', 'g'), '')::numeric;
    RETURN num_val;
  END IF;
...
-- Step 2: Drop all existing overloads to ensure clean replacement
DO $$
DECLARE
  r RECORD;
BEGIN
  FOR r IN
    ...
    FROM pg_proc
    WHERE proname = 'search_free_data_builder'
...
-- Step 3: Create the function with ONLY revenue filter changed
...
BEGIN
  ...
  WHERE fd.entity_type = p_entity_type
    ...
    -- SENIORITY FILTER - v2.3 inline logic (WORKING - UNCHANGED)
    ...
    -- DEPARTMENT FILTER - v2.3 inline logic (WORKING - UNCHANGED)
    ...
    -- COMPANY SIZE FILTER - v2.3 (WORKING - UNCHANGED)
    ...
    -- v2.10 FIX: Company revenue filter with numeric parsing
    ...
    -- PROSPECT DATA FILTERS - v2.7 FIXED (UNCHANGED)
...
-- Step 4: Fail-fast assertions to prevent duplicates
DO $$
DECLARE
  func_count integer;
  param_count integer;
BEGIN
  ...
  IF func_count != 1 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: Expected exactly 1 search_free_data_builder function, found %', func_count;
  END IF;
  
  -- Check parameter count is 29
  ...
  IF param_count != 29 THEN
    RAISE EXCEPTION 'ASSERTION FAILED: Expected 29 parameters, found %', param_count;
  END IF;
  
  RAISE NOTICE 'v2.10 ASSERTIONS PASSED: 1 function with 29 parameters';
END $$;