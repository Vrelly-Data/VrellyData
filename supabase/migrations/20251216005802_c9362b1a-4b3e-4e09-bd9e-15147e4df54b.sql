-- Drop all existing versions of the function to eliminate ambiguity
DROP FUNCTION IF EXISTS public.search_free_data_with_filters(entity_type, text[], text[], text[], text, text[], text[], text[], text[], text[], text[], boolean, boolean, boolean, boolean, boolean, boolean, integer, integer);
DROP FUNCTION IF EXISTS public.search_free_data_with_filters(entity_type, text[], text[], text[], text, text[], text[], text[], text[], text[], text[], boolean, boolean, boolean, boolean, boolean, boolean, text[], text[], text[], text[], integer, integer);
DROP FUNCTION IF EXISTS public.search_free_data_with_filters(entity_type, text[], text[], text[], text, text[], text[], text[], text[], text[], text[], boolean, boolean, boolean, boolean, boolean, boolean, text[], text[], text[], text[], text[], text[], integer, integer);
...
-- Create the unified function with all parameters and fixed net worth/income range matching
...
BEGIN
  ...
  WHERE fd.entity_type = p_entity_type
    ...
    -- Data format: "$1", "$750", "$1000" etc (representing thousands)
    -- UI ranges: Under $100K, $100K - $500K, $500K - $1M, $1M - $5M, $5M - $10M, $10M - $50M, $50M+
    AND (
      p_net_worth IS NULL OR array_length(p_net_worth, 1) IS NULL
      OR (
        -- Parse the numeric value from netWorth field (e.g., "$100" -> 100, "$1000" -> 1000)
        CASE 
    ...
    -- Income filter with RANGE-BASED matching (same pattern as net worth)
    AND (
