-- ============================================================
-- v2.8: Fix Company Revenue Filter (add companyRevenue field)
-- GUARDED MIGRATION: Drops all overloads first to prevent conflicts
-- Changes: Add 'companyRevenue' to COALESCE in revenue filter (2 places)
-- ============================================================

-- Step 1: Drop ALL overloads of search_free_data_builder to prevent conflicts
DO $$
DECLARE
  r RECORD;
BEGIN
...
-- Step 2: Recreate the canonical function with companyRevenue fix
...
BEGIN
  ...
  WHERE fd.entity_type = p_entity_type
    ...
    -- v2.8 FIX: Company revenue filter (added companyRevenue as first priority)
