...
-- CANONICAL FUNCTION RESTORATION: Use Jan 13 working logic + add p_technologies
-- This migration:
-- 1) Drops ALL existing overloads of search_free_data_builder
-- 2) Creates ONE canonical function with the proven working logic
-- 3) Adds p_technologies parameter with proper JSON array handling
-- 4) Forces schema cache refresh

-- Step 1: Drop ALL existing overloads dynamically
DO $$
DECLARE
  func_record RECORD;
BEGIN
...
-- Step 2: Create ONE canonical function with Jan 13 working logic + p_technologies
...
BEGIN
  ...
  WHERE
    ...
    -- COMPANY SIZE: Parse numeric employee count and match against ranges
    ...
    -- COMPANY REVENUE: Parse numeric revenue and match against ranges
    ...
    -- TECHNOLOGIES: Handle as JSON array with case-insensitive substring match
    ...
    -- COMPANY LINKEDIN: Check both field names
  ...
  WHERE
    ...
    AND (p_has_company_phone IS NULL OR p_has_company_phone = false OR 
         coalesce(fd.entity_data->>'companyPhone', '') <> '')
    -- Company LinkedIn (same as count)
    AND (p_has_company_linkedin IS NULL OR p_has_company_linkedin = false OR 
         coalesce(fd.entity_data->>'companyLinkedin', '') <> '' OR
         coalesce(fd.entity_data->>'companyLinkedinUrl', '') <> '')
    AND (p_has_company_facebook IS NULL OR p_has_company_facebook = false OR 
