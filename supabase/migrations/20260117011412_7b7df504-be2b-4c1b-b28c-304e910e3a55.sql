...
-- ============================================================
-- FIX 5 BROKEN FILTERS v2.3
-- Step 1: DROP existing function
-- Step 2: CREATE with same signature + fixed logic
-- ============================================================

-- DROP the existing 29-parameter function
...
-- Recreate with same signature but fixed logic
...
BEGIN
  ...
  WHERE fd.entity_type = p_entity_type
    ...
    -- SENIORITY FILTER - FIXED v2.3: Added president, head of
    ...
    -- DEPARTMENT FILTER - FIXED v2.3: Added customer success, product
    ...
    -- COMPANY SIZE FILTER - FIXED v2.3: Changed employeeCount -> companySize
    ...
    -- Company revenue filter (WORKING)
    ...
    -- INCOME FILTER - FIXED v2.3: Only match records with income data
    ...
    -- NET WORTH FILTER - FIXED v2.3: Handle negative values
  ...
  -- Return results with total count (columns in correct order)
  RETURN QUERY
  SELECT 
...
COMMENT ON FUNCTION public.search_free_data_builder IS 'v2.3 - Fixed: Company Size (companySize field), Seniority (president/head of), Department (customer success/product), Income (data-only), Net Worth (negative values). 29 parameters.';
