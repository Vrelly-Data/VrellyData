...
BEGIN
  ...
  WHERE fd.entity_type = p_entity_type
    ...
    -- Prospect data filters (FIXED: linkedin and companyLinkedin field names)
    ...
    AND (p_exclude_keywords IS NULL OR array_length(p_exclude_keywords, 1) IS NULL OR NOT EXISTS (
      ...
      WHERE fd.entity_data->>'firstName' ILIKE '%' || kw || '%' OR fd.entity_data->>'lastName' ILIKE '%' || kw || '%' OR fd.entity_data->>'title' ILIKE '%' || kw || '%' OR fd.entity_data->>'company' ILIKE '%' || kw || '%' OR fd.entity_data->>'companyName' ILIKE '%' || kw || '%' OR fd.entity_data->>'industry' ILIKE '%' || kw || '%'
    ))
