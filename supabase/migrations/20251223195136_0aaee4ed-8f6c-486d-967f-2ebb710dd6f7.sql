-- Update search_free_data_with_filters to search all relevant text fields
    ...
    AND (
      ...
      OR EXISTS (
        ...
        WHERE 
          ...
          -- NEW FIELDS for comprehensive keyword search
...
-- Also update search_free_data_keywords function
CREATE OR REPLACE FUNCTION public.search_free_data_keywords(p_entity_type entity_type, p_keywords text[], p_limit integer DEFAULT 100, p_offset integer DEFAULT 0)
 RETURNS TABLE(id uuid, entity_type entity_type, entity_external_id text, entity_data jsonb, created_at timestamp with time zone, total_count bigint)
 LANGUAGE plpgsql
 STABLE SECURITY DEFINER
 SET search_path TO 'public'
