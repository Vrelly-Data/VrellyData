-- Separate Owner and Founder seniority filters
-- Owner should NOT match Founder, and Founder gets its own case
    ...
    LOOP
      ...
      CASE lower(seniority_value)
        ...
        WHEN 'entry' THEN
          ...
          END IF;
        WHEN 'owner' THEN
          -- FIXED: Owner no longer includes founder
          IF seniority_lower IN ('owner', 'co-owner', 'coowner', 'proprietor', 'partner', 'principal') THEN
            RETURN TRUE;
          END IF;
        WHEN 'founder' THEN
          -- NEW: Separate case for Founder
          IF seniority_lower IN ('founder', 'co-founder', 'cofounder') THEN
            RETURN TRUE;
          END IF;
        ELSE
          NULL;
  ...
  LOOP
    CASE lower(seniority_value)
      ...
      WHEN 'entry' THEN
        ...
        END IF;
      WHEN 'owner' THEN
        -- FIXED: Owner title regex no longer includes founder
        IF title_lower ~ '(^|[^a-z])(owner|partner|principal|proprietor)[^a-z]|^(owner|partner|principal|proprietor)$' THEN
          RETURN TRUE;
        END IF;
      WHEN 'founder' THEN
        -- NEW: Founder title regex fallback
        IF title_lower ~ '(^|[^a-z])(founder|co-founder|cofounder)[^a-z]|^(founder|co-founder|cofounder)$' THEN
          RETURN TRUE;
        END IF;
      ELSE
        NULL;
