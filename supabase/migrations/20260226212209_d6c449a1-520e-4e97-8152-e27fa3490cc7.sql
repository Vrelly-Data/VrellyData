...
BEGIN
  ...
  IF has_filters THEN
    -- Use planner estimate instead of actual count to avoid timeouts
    explain_sql := 'EXPLAIN (FORMAT JSON) SELECT 1 FROM public.free_data fd ' || where_clause;
    EXECUTE explain_sql INTO explain_result
  ...
  -- STEP 2: Get paginated data (no window function)
