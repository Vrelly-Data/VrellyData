...
-- Update the search_free_data_with_filters function to use range-based matching for net worth
-- This handles values like '$100', '$500', '-$20' (representing thousands of dollars)
...
BEGIN
  ...
  WHERE fd.entity_type = p_entity_type
    ...
    -- Data stored as '$100', '$500', '-$20' (representing thousands of dollars)
    AND (
      ...
      OR (
        ...
        AND (
          -- Under $100K: numeric value < 100
          (
            'Under $100K' = ANY(p_net_worth)
            AND COALESCE(NULLIF(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[^0-9\-]', '', 'g'), '')::integer, 0) < 100
          )
          OR
          -- $100K - $500K: 100 <= numeric value < 500
          (
            '$100K - $500K' = ANY(p_net_worth)
            AND COALESCE(NULLIF(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[^0-9\-]', '', 'g'), '')::integer, 0) >= 100
            AND COALESCE(NULLIF(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[^0-9\-]', '', 'g'), '')::integer, 0) < 500
          )
          OR
          -- $500K - $1M: 500 <= numeric value < 1000
          (
            '$500K - $1M' = ANY(p_net_worth)
            AND COALESCE(NULLIF(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[^0-9\-]', '', 'g'), '')::integer, 0) >= 500
            AND COALESCE(NULLIF(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[^0-9\-]', '', 'g'), '')::integer, 0) < 1000
          )
          OR
          -- $1M - $5M: 1000 <= numeric value < 5000
          (
            '$1M - $5M' = ANY(p_net_worth)
            AND COALESCE(NULLIF(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[^0-9\-]', '', 'g'), '')::integer, 0) >= 1000
            AND COALESCE(NULLIF(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[^0-9\-]', '', 'g'), '')::integer, 0) < 5000
          )
          OR
          -- $5M - $10M: 5000 <= numeric value < 10000
          (
            '$5M - $10M' = ANY(p_net_worth)
            AND COALESCE(NULLIF(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[^0-9\-]', '', 'g'), '')::integer, 0) >= 5000
            AND COALESCE(NULLIF(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[^0-9\-]', '', 'g'), '')::integer, 0) < 10000
          )
          OR
          -- $10M+: numeric value >= 10000
          (
            '$10M+' = ANY(p_net_worth)
            AND COALESCE(NULLIF(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[^0-9\-]', '', 'g'), '')::integer, 0) >= 10000
          )
  ...
  WHERE fd.entity_type = p_entity_type
    ...
    -- Net worth filter with RANGE-BASED matching (same logic as count query)
    AND (
      p_net_worth IS NULL OR array_length(p_net_worth, 1) IS NULL
      OR (
        fd.entity_data->>'netWorth' IS NOT NULL 
