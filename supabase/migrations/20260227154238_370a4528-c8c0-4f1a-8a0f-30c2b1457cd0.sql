...
-- Function 1: Results only (fast, no count)
...
BEGIN
  ...
  IF p_exclude_job_titles IS NOT NULL AND array_length(p_exclude_job_titles, 1) IS NOT NULL THEN
    where_clause := where_clause || ' AND NOT EXISTS (SELECT 1 FROM unnest($31) jt WHERE LOWER(fd.entity_data->>''title'') ILIKE ''%''||LOWER(jt)||''%'')';
  END IF;
...
-- Function 2: Count only (smart tiered strategy)
