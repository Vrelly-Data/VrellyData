-- ============================================================
-- v2.9: EXACT v2.7 Stable + companyRevenue Field Fix
-- This migration restores the EXACT v2.7 function with ONLY 
-- 2 lines changed to add companyRevenue to the revenue COALESCE
-- ============================================================

-- Guard: Drop all overloads to ensure clean replacement
DO $$
DECLARE
  r RECORD;
BEGIN
...
-- Create the EXACT v2.7 function with companyRevenue fix (2 lines changed: 164 and 429)
...
BEGIN
  ...
  WHERE fd.entity_type = p_entity_type
    ...
    -- Company revenue filter - v2.9 FIX: Added companyRevenue field
    AND (p_company_revenue IS NULL OR array_length(p_company_revenue, 1) IS NULL OR EXISTS (
      ...
      WHERE LOWER(COALESCE(fd.entity_data->>'companyRevenue', fd.entity_data->>'revenue', fd.entity_data->>'annualRevenue', '')) ILIKE '%' || LOWER(cr) || '%'
    ))
    -- Technologies filter
    ...
    -- PROSPECT DATA FILTERS - v2.7 FIXED
  ...
  WHERE fd.entity_type = p_entity_type
    ...
    -- v2.9 FIX: Added companyRevenue field
    AND (p_company_revenue IS NULL OR array_length(p_company_revenue, 1) IS NULL OR EXISTS (
      SELECT 1 FROM unnest(p_company_revenue) cr
      WHERE LOWER(COALESCE(fd.entity_data->>'companyRevenue', fd.entity_data->>'revenue', fd.entity_data->>'annualRevenue', '')) ILIKE '%' || LOWER(cr) || '%'
    ))
