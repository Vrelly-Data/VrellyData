-- Remove redundant LOWER() wrappers from column side of ILIKE expressions.
-- ILIKE is already case-insensitive, so LOWER() on the column prevents
-- PostgreSQL from using the existing pg_trgm GIN indexes, causing full
-- sequential scans and search timeouts on the free_data table.
...
BEGIN
  ...
  IF p_exclude_keywords IS NOT NULL AND array_length(p_exclude_keywords, 1) IS NOT NULL THEN
    where_clause := where_clause || ' AND NOT EXISTS (SELECT 1 FROM unnest($30) kw WHERE fd.entity_data->>''firstName'' ILIKE ''%''||kw||''%'' OR fd.entity_data->>''lastName'' ILIKE ''%''||kw||''%'' OR fd.entity_data->>''title'' ILIKE ''%''||kw||''%'' OR fd.entity_data->>''company'' ILIKE ''%''||kw||''%'' OR fd.entity_data->>''companyName'' ILIKE ''%''||kw||''%'' OR fd.entity_data->>''industry'' ILIKE ''%''||kw||''%'')';
  END IF;
...
-- Function 3: search_free_data_builder
