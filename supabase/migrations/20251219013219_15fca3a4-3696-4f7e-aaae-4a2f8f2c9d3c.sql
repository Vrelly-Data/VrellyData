-- Helper: title-only seniority matching (single source of truth)
...
DECLARE
  ...
  wants_c_level boolean := EXISTS (SELECT 1 FROM unnest(p_seniority) s WHERE lower(s) IN ('cxo', 'c-level', 'c level'));
  wants_president boolean := EXISTS (SELECT 1 FROM unnest(p_seniority) s WHERE lower(s) = 'president');
  wants_vp boolean := EXISTS (SELECT 1 FROM unnest(p_seniority) s WHERE lower(s) IN ('vp', 'vice president'));
  wants_director boolean := EXISTS (SELECT 1 FROM unnest(p_seniority) s WHERE lower(s) = 'director');
  wants_head_of boolean := EXISTS (SELECT 1 FROM unnest(p_seniority) s WHERE lower(s) = 'head of');
  wants_manager boolean := EXISTS (SELECT 1 FROM unnest(p_seniority) s WHERE lower(s) = 'manager');
  wants_ic boolean := EXISTS (SELECT 1 FROM unnest(p_seniority) s WHERE lower(s) = 'individual contributor');
  ...
  is_manager boolean;
BEGIN
  -- If no seniority filter selected, allow all rows
  IF p_seniority IS NULL OR array_length(p_seniority, 1) IS NULL THEN
    RETURN true;
  END IF;

  -- TITLE-ONLY classification (do NOT use entity_data->>'seniority')
  -- C-level: strict word-boundary acronyms + Chief ... Officer/Chief Executive
  is_c_level := (
    t ~* '(^|[^a-zA-Z])(CEO|CTO|CFO|CMO|CISO|CIO|CSO|COO|CHRO|CRO)([^a-zA-Z]|$)'
    OR t ILIKE 'Chief %Officer%'
    OR t ILIKE '% Chief %Officer%'
    OR t ILIKE 'Chief Executive%'
    OR t ILIKE '% Chief Executive%'
  );

  -- President: title contains President but NOT Vice President
  is_president := (
    t ~* '(^|[^a-zA-Z])President([^a-zA-Z]|$)'
    AND t NOT ILIKE '%Vice President%'
  );

  -- VP: Vice President phrase or VP-family acronyms (VP/SVP/EVP/AVP)
  is_vp := (
    t ILIKE '%Vice President%'
    OR t ~* '(^|[^a-zA-Z])(VP|SVP|EVP|AVP)([^a-zA-Z]|$)'
  );

  -- Director: Director word but not Managing Director
  is_director := (
    t ~* '(^|[^a-zA-Z])Director([^a-zA-Z]|$)'
    AND t NOT ILIKE '%Managing Director%'
  );

  -- Head of: common patterns
  is_head_of := (
    t ILIKE '%Head of%'
    OR t ILIKE '%Principal%'
    OR t ILIKE '%Managing Director%'
  );

  -- Manager
  is_manager := (t ~* '(^|[^a-zA-Z])Manager([^a-zA-Z]|$)');

  -- Evaluate requested seniority buckets as OR (union of buckets)
  IF (wants_c_level AND is_c_level)
     OR (wants_president AND is_president)
     OR (wants_vp AND is_vp)
     OR (wants_director AND is_director)
     OR (wants_head_of AND is_head_of)
     OR (wants_manager AND is_manager)
  THEN
    RETURN true;
  END IF;

  -- Individual contributor: anything not classified as a senior bucket above
  IF wants_ic THEN
    RETURN NOT (is_c_level OR is_president OR is_vp OR is_director OR is_head_of OR is_manager);
  END IF;
...
-- Main search function: keep all existing filter behavior, but replace seniority logic with title-only matching
CREATE OR REPLACE FUNCTION public.search_free_data_with_filters(
  ...
  p_person_city text[] DEFAULT NULL::text[],
  p_person_country text[] DEFAULT NULL::text[],
  p_company_city text[] DEFAULT NULL::text[],
  p_company_country text[] DEFAULT NULL::text[],
