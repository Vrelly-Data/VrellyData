...
-- =============================================
-- v3.10 Performance Optimization
-- Expression indexes + single-pass COUNT/SELECT
-- WHERE clause is IDENTICAL to v3.9
-- =============================================

-- 1. Expression indexes for frequently filtered JSONB fields
CREATE INDEX IF NOT EXISTS idx_free_data_industry ON public.free_data (LOWER(entity_data->>'industry')) WHERE entity_data->>'industry' IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_free_data_title ON public.free_data (LOWER(entity_data->>'title')) WHERE entity_data->>'title' IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_free_data_city ON public.free_data (LOWER(COALESCE(entity_data->>'city', entity_data->>'personCity', entity_data->>'companyCity', '')));
CREATE INDEX IF NOT EXISTS idx_free_data_country ON public.free_data (LOWER(COALESCE(entity_data->>'country', entity_data->>'personCountry', entity_data->>'companyCountry', '')));
CREATE INDEX IF NOT EXISTS idx_free_data_seniority ON public.free_data (LOWER(entity_data->>'seniority')) WHERE entity_data->>'seniority' IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_free_data_department ON public.free_data (LOWER(entity_data->>'department')) WHERE entity_data->>'department' IS NOT NULL;
CREATE INDEX IF NOT EXISTS idx_free_data_gender ON public.free_data (LOWER(entity_data->>'gender')) WHERE entity_data->>'gender' IS NOT NULL;

-- 2. Single-pass rewrite of search_free_data_builder
-- Same 37 parameters, same WHERE clause, just uses count(*) OVER() instead of separate COUNT
