...
-- Update the search_free_data_with_filters function to:
-- 1. Fix company revenue filtering with range-based matching
-- 2. Fix company city/country to also check city/country fields for company entities
...
BEGIN
  ...
  WHERE fd.entity_type = p_entity_type
    ...
    -- Company City filter - also check city field for company entities
    AND (
      ...
      OR EXISTS (
        ...
        WHERE (fd.entity_data->>'companyCity') ILIKE '%' || cc || '%'
           OR (fd.entity_data->>'city') ILIKE '%' || cc || '%'
      )
    )
    -- Company Country filter - also check country field for company entities
    AND (
      ...
      OR EXISTS (
        ...
        WHERE (fd.entity_data->>'companyCountry') ILIKE '%' || cco || '%'
           OR (fd.entity_data->>'country') ILIKE '%' || cco || '%'
      )
    )
      ...
      OR (
        ...
        AND (
          (EXISTS (SELECT 1 FROM unnest(p_net_worth) nw WHERE nw = 'Under $100K')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[$,]', '', 'g') AS numeric) < 100000)
          OR
          (EXISTS (SELECT 1 FROM unnest(p_net_worth) nw WHERE nw = '$100K - $500K')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[$,]', '', 'g') AS numeric) >= 100000
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[$,]', '', 'g') AS numeric) < 500000)
          OR
          (EXISTS (SELECT 1 FROM unnest(p_net_worth) nw WHERE nw = '$500K - $1M')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[$,]', '', 'g') AS numeric) >= 500000
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[$,]', '', 'g') AS numeric) < 1000000)
          OR
          (EXISTS (SELECT 1 FROM unnest(p_net_worth) nw WHERE nw = '$1M - $5M')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[$,]', '', 'g') AS numeric) >= 1000000
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[$,]', '', 'g') AS numeric) < 5000000)
          OR
          (EXISTS (SELECT 1 FROM unnest(p_net_worth) nw WHERE nw = '$5M - $10M')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[$,]', '', 'g') AS numeric) >= 5000000
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[$,]', '', 'g') AS numeric) < 10000000)
          OR
          (EXISTS (SELECT 1 FROM unnest(p_net_worth) nw WHERE nw = '$10M - $50M')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[$,]', '', 'g') AS numeric) >= 10000000
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'netWorth', '[$,]', '', 'g') AS numeric) < 50000000)
          OR
      ...
      OR (
        ...
        AND (
          (EXISTS (SELECT 1 FROM unnest(p_income) inc WHERE inc = 'Under $50K')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'incomeRange', '[$,]', '', 'g') AS numeric) < 50000)
          OR
          (EXISTS (SELECT 1 FROM unnest(p_income) inc WHERE inc = '$50K - $100K')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'incomeRange', '[$,]', '', 'g') AS numeric) >= 50000
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'incomeRange', '[$,]', '', 'g') AS numeric) < 100000)
          OR
          (EXISTS (SELECT 1 FROM unnest(p_income) inc WHERE inc = '$100K - $200K')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'incomeRange', '[$,]', '', 'g') AS numeric) >= 100000
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'incomeRange', '[$,]', '', 'g') AS numeric) < 200000)
          OR
          (EXISTS (SELECT 1 FROM unnest(p_income) inc WHERE inc = '$200K - $500K')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'incomeRange', '[$,]', '', 'g') AS numeric) >= 200000
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'incomeRange', '[$,]', '', 'g') AS numeric) < 500000)
          OR
          (EXISTS (SELECT 1 FROM unnest(p_income) inc WHERE inc = '$500K - $1M')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'incomeRange', '[$,]', '', 'g') AS numeric) >= 500000
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'incomeRange', '[$,]', '', 'g') AS numeric) < 1000000)
          OR
          (EXISTS (SELECT 1 FROM unnest(p_income) inc WHERE inc = '$1M+')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'incomeRange', '[$,]', '', 'g') AS numeric) >= 1000000)
        )
      )
    )
    -- Company Revenue filter - RANGE-BASED matching for numeric and text labels
    AND (
      p_company_revenue IS NULL OR array_length(p_company_revenue, 1) IS NULL
      OR (
        -- Parse revenue as numeric (handles "459000000", "$10,000,000", etc.)
        CASE 
          WHEN fd.entity_data->>'companyRevenue' IS NOT NULL 
               AND fd.entity_data->>'companyRevenue' ~ '^[\$]?[0-9,]+$'
          THEN 
            CAST(REGEXP_REPLACE(fd.entity_data->>'companyRevenue', '[$,]', '', 'g') AS numeric)
          ELSE NULL
        END IS NOT NULL
        AND (
          (EXISTS (SELECT 1 FROM unnest(p_company_revenue) cr WHERE cr = 'Under $1M')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'companyRevenue', '[$,]', '', 'g') AS numeric) < 1000000)
          OR
          (EXISTS (SELECT 1 FROM unnest(p_company_revenue) cr WHERE cr = '$1M - $10M')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'companyRevenue', '[$,]', '', 'g') AS numeric) >= 1000000
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'companyRevenue', '[$,]', '', 'g') AS numeric) < 10000000)
          OR
          (EXISTS (SELECT 1 FROM unnest(p_company_revenue) cr WHERE cr = '$10M - $50M')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'companyRevenue', '[$,]', '', 'g') AS numeric) >= 10000000
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'companyRevenue', '[$,]', '', 'g') AS numeric) < 50000000)
          OR
          (EXISTS (SELECT 1 FROM unnest(p_company_revenue) cr WHERE cr = '$50M - $100M')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'companyRevenue', '[$,]', '', 'g') AS numeric) >= 50000000
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'companyRevenue', '[$,]', '', 'g') AS numeric) < 100000000)
          OR
          (EXISTS (SELECT 1 FROM unnest(p_company_revenue) cr WHERE cr = '$100M - $500M')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'companyRevenue', '[$,]', '', 'g') AS numeric) >= 100000000
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'companyRevenue', '[$,]', '', 'g') AS numeric) < 500000000)
          OR
          (EXISTS (SELECT 1 FROM unnest(p_company_revenue) cr WHERE cr = '$500M - $1B')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'companyRevenue', '[$,]', '', 'g') AS numeric) >= 500000000
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'companyRevenue', '[$,]', '', 'g') AS numeric) < 1000000000)
          OR
          (EXISTS (SELECT 1 FROM unnest(p_company_revenue) cr WHERE cr = '$1B+')
           AND CAST(REGEXP_REPLACE(fd.entity_data->>'companyRevenue', '[$,]', '', 'g') AS numeric) >= 1000000000)
        )
      )
      -- Also match text labels like "5 Million To 10 Million", "Under 1 Million", etc.
      OR (
        (EXISTS (SELECT 1 FROM unnest(p_company_revenue) cr WHERE cr = 'Under $1M')
         AND (fd.entity_data->>'companyRevenue') ILIKE '%Under%Million%')
        OR
        (EXISTS (SELECT 1 FROM unnest(p_company_revenue) cr WHERE cr = '$1M - $10M')
         AND ((fd.entity_data->>'companyRevenue') ILIKE '%1 Million To%' 
              OR (fd.entity_data->>'companyRevenue') ILIKE '%5 Million To 10%'))
        OR
        (EXISTS (SELECT 1 FROM unnest(p_company_revenue) cr WHERE cr = '$10M - $50M')
         AND ((fd.entity_data->>'companyRevenue') ILIKE '%10 Million To%'
              OR (fd.entity_data->>'companyRevenue') ILIKE '%25 Million To 50%'))
        OR
        (EXISTS (SELECT 1 FROM unnest(p_company_revenue) cr WHERE cr = '$50M - $100M')
         AND (fd.entity_data->>'companyRevenue') ILIKE '%50 Million To%')
        OR
        (EXISTS (SELECT 1 FROM unnest(p_company_revenue) cr WHERE cr = '$100M - $500M')
         AND ((fd.entity_data->>'companyRevenue') ILIKE '%100 Million%'
              OR (fd.entity_data->>'companyRevenue') ILIKE '%250 Million%'))
        OR
        (EXISTS (SELECT 1 FROM unnest(p_company_revenue) cr WHERE cr = '$500M - $1B')
         AND (fd.entity_data->>'companyRevenue') ILIKE '%500 Million%')
        OR
        (EXISTS (SELECT 1 FROM unnest(p_company_revenue) cr WHERE cr = '$1B+')
         AND ((fd.entity_data->>'companyRevenue') ILIKE '%Billion%'
              OR (fd.entity_data->>'companyRevenue') ILIKE '%Over 1%'))
      )
    )
    -- Has personal email
    AND (
