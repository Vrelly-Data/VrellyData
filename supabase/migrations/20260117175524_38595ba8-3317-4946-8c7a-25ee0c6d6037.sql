-- ============================================================
-- v3.1: COMPANY SIZE BUCKETS FIX + INDIVIDUAL CONTRIBUTOR
-- Changes:
-- 1. Split '5001+' into '5001-10000' and '10000+' buckets
-- 2. Add 'Individual Contributor' seniority mapping for 'Staff'
-- All other logic is IDENTICAL to v3.0
-- ============================================================

-- Step 1: Drop all existing overloads to ensure clean replacement
DO $$
DECLARE
  r RECORD;
BEGIN
...
-- Step 2: Create the function with v3.1 changes
...
BEGIN
  ...
  WHERE fd.entity_type = p_entity_type
    ...
    -- SENIORITY FILTER - v3.1: Added Individual Contributor
    AND (p_seniority_levels IS NULL OR array_length(p_seniority_levels, 1) IS NULL OR EXISTS (
      ...
      WHERE CASE LOWER(sl)
        ...
        -- v3.1 NEW: Individual Contributor maps to 'Staff'
    ...
    -- COMPANY SIZE FILTER - v3.1: Split 5001+ into 5001-10000 and 10000+
    AND (p_company_size_ranges IS NULL OR array_length(p_company_size_ranges, 1) IS NULL OR EXISTS (
      ...
      WHERE CASE csr
        ...
        -- v3.1 FIX: Split into two buckets
    ...
    -- PROSPECT DATA FILTERS (UNCHANGED)
...
-- Step 3: Verify exactly one function exists with 29 parameters
DO $$
DECLARE
  func_count integer;
  param_count integer;
BEGIN
  ...
  IF func_count != 1 THEN
    RAISE EXCEPTION 'Expected exactly 1 search_free_data_builder function, found %', func_count;
  END IF;
  ...
  IF param_count != 29 THEN
    RAISE EXCEPTION 'Expected 29 parameters, found %', param_count;
  END IF;
  
  RAISE NOTICE 'v3.1 Migration successful: 1 function with 29 parameters';
END $$;